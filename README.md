...
En vez de instalar MySQL, Python y Node manualmente, puedes levantar todo el stack con Docker en un solo comando. El frontend corre en modo desarrollo con hot-reload (Vite), igual que con `npm run dev`.
...
La primera vez que arranca, el contenedor `mysql` crea un volumen Docker vacío (`mysql_data_docker`) y el propio `servidor.py` ejecuta `base_de_datos.sql` para crear tablas y datos de prueba — igual que cuando corres el backend localmente.
...
### Notas
- El código de `backend/` y `frontend/` se monta como volumen, así que los cambios que hagas se reflejan sin reconstruir la imagen (solo necesitas reconstruir si agregas dependencias nuevas en `requirements.txt` o `package.json`).
- Las claves de FatSecret, Wger y SMTP viven en `.env` (ignorado por git). `configuracion_bd.py` ahora lee `DB_HOST`, `DB_PORT`, `DB_USER`, `DB_PASSWORD` y `DB_NAME` desde variables de entorno, con los mismos valores por defecto de antes (`localhost`/`root`/`root`) para que siga funcionando igual si corres el proyecto sin Docker.
