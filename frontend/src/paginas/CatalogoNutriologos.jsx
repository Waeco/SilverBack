...
import { obtenerNutriologos, obtenerUsuario, enviarSolicitud } from '../servicios/ApiServicio'
import { Search, Stethoscope, MapPin, Award, ChevronLeft, ChevronRight, Loader2, BadgeCheck, UserPlus, UserCheck, Ban, Clock, CheckCircle2 } from 'lucide-react'
...
export default function CatalogoNutriologos() {
  ...
  useEffect(() => {
    if (!usuario || usuario.rol !== 'atleta') return
    const cargarPerfil = async () => {
      try {
        const r = await obtenerUsuario(usuario.id_usuario)
        setPerfilPaciente(r.data.usuario.perfil || null)
      } catch {
  ...
  const manejarSolicitar = async (idNutriologo) => {
    setEnviando(idNutriologo)
    setMensaje(null)
    try {
      const idPaciente = perfilPaciente.id_paciente
      await enviarSolicitud(idPaciente, idNutriologo)
      setSolicitados((prev) => new Set(prev).add(idNutriologo))
      setMensaje({ tipo: 'exito', texto: 'Solicitud enviada correctamente.' })
    } catch (err) {
      setMensaje({ tipo: 'error', texto: err.response?.data?.detail || 'Error al enviar solicitud' })
    } finally {
      setEnviando(null)
    }
  }

  const tieneNutriologo = perfilPaciente && perfilPaciente.id_nutriologo_asignado
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      ...
      {mensaje && (
        <div className={`mb-4 px-4 py-2 rounded-lg text-sm ${
          mensaje.tipo === 'exito' ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'
        }`}>
          {mensaje.texto}
        </div>
      )}
      ...
      {tieneNutriologo && (
        <div className="mb-4 px-4 py-3 rounded-lg bg-blue-500/10 text-blue-400 border border-blue-500/20 flex items-center gap-2">
          <UserCheck className="w-4 h-4" />
          Ya tienes un nutriólogo asignado. Puedes gestionarlo desde tu perfil.
        </div>
      )}
      ...
      {cargando ? (
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-2">
            {nutriologos.map((n, i) => {
              const yaSolicitado = solicitados.has(n.id_nutriologo)
              return (
                <motion.div
                  key={n.id_nutriologo}
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {usuario?.rol === 'atleta' && !tieneNutriologo && !yaSolicitado && (
                        <button
                          onClick={() => manejarSolicitar(n.id_nutriologo)}
                          disabled={enviando === n.id_nutriologo}
                          className="btn-primary text-xs flex items-center gap-1.5 px-3 py-1.5 mt-3"
                        >
                          {enviando === n.id_nutriologo ? (
